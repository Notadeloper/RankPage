...
public class HW7Test extends GenericTest {
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("filter")) try {
        startTest("filter", "ec1", 5);
        try {
            String response = FlameSubmit.submit("localhost:9000", "tests/FlameFilter.jar", "cis5550.test.FlameFilter", new String[] {});
    ...
    if (tests.contains("mapPartitions")) try {
        startTest("mapPartitions", "ec2", 5);
        try {
            String response = FlameSubmit.submit("localhost:9000", "tests/FlameMapPartitions.jar", "cis5550.test.FlameMapPartitions", new String[] {});
            System.out.println(response);
            testSucceeded();
          } catch (ConnectException ce) {
            testFailed("We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
    ...
    if (tests.contains("cogroup")) try {
        startTest("cogroup", "ec3", 5);
        try {
            String response = FlameSubmit.submit("localhost:9000", "tests/FlameCogroup.jar", "cis5550.test.FlameCogroup", new String[] {});
            System.out.println(response);
            testSucceeded();
          } catch (ConnectException ce) {
            testFailed("We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
