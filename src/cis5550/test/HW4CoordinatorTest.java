...
public class HW4CoordinatorTest extends GenericTest {
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("ping-1")) try {
        startTest("ping-1", "custom test", 5);
        ...
        if (r.statusCode != 200)
          testFailed("The server returned a "+r.statusCode+" response, but we were expecting a 200 OK. Here is what was in the body:\n\n"+dump(r.body));
        if (!r.body().equals("1\nabcde,127.0.0.1:1234\n"))
            testFailed("bad body");
        s.close();
        testSucceeded();
      } catch (Exception e) { testFailed("An exception occurred: "+e, false); e.printStackTrace(); }
    
    if (tests.contains("ping-2")) try {
        startTest("ping-2", "custom test", 5);
        Socket s = openSocket(8000);
        PrintWriter out = new PrintWriter(s.getOutputStream());
        out.print("GET /ping?id=abcde&port=4321 HTTP/1.1\r\nHost: localhost\r\n\r\n");
        out.flush();
        Response r = readAndCheckResponse(s, "response");
        if (r.statusCode != 200)
          testFailed("The server returned a "+r.statusCode+" response, but we were expecting a 200 OK. Here is what was in the body:\n\n"+dump(r.body));
        ...
        if (r.statusCode != 200)
          testFailed("The server returned a "+r.statusCode+" response, but we were expecting a 200 OK. Here is what was in the body:\n\n"+dump(r.body));
        if (!r.body().equals("1\nabcde,127.0.0.1:4321\n"))
            testFailed("bad body");
        s.close();
        testSucceeded();
      } catch (Exception e) { testFailed("An exception occurred: "+e, false); e.printStackTrace(); }
    
    if (tests.contains("ping-3")) try {
        startTest("ping-3", "custom test", 5);
        Socket s = openSocket(8000);
        PrintWriter out = new PrintWriter(s.getOutputStream());
    ...
    if (tests.contains("ping-4")) try {
        startTest("ping-4", "custom test", 5);
        Socket s = openSocket(8000);
        PrintWriter out = new PrintWriter(s.getOutputStream());
    ...
    if (tests.contains("ping-5")) try {
        startTest("ping-5", "custom test", 5);
        Socket s = openSocket(8000);
        PrintWriter out = new PrintWriter(s.getOutputStream());
        out.print("GET /pingHTTP/1.1\r\nHost: localhost\r\n\r\n");
        out.flush();
        Response r = readAndCheckResponse(s, "response");
        if (r.statusCode != 400)
          testFailed("The server returned a "+r.statusCode+" response, but we were expecting a 200 OK. Here is what was in the body:\n\n"+dump(r.body));
    ...
    if (tests.contains("table")) try {
      ...
      for (int i = 0; i < workers.size(); i++) {
          ...
          if (!r.body().contains(worker))
              testFailed("In response to our GET /, the server did return a HTML table, but we couldn't find an entry for '"+worker+"', which is the ID of a worker we registered. Here is what was in the body:\n\n"+dump(r.body));
          if (!r.body().contains("http://127.0.0.1:" + port + "/"))
              testFailed("Didn't contain the hyperlink");
      }
