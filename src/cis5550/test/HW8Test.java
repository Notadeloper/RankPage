...
public class HW8Test extends GenericTest {
  ...
  void prompt() {
    ...
    System.out.println(
        "  rm -rf worker1; java -cp lib/kvs.jar:lib/webserver.jar cis5550.kvs.Worker 8001 worker1 localhost:8000");
    System.out.println(
        "  java -cp lib/kvs.jar:lib/webserver.jar:lib/flame.jar cis5550.flame.Coordinator 9000 localhost:8000");
    System.out
        .println("  java -cp lib/kvs.jar:lib/webserver.jar:lib/flame.jar cis5550.flame.Worker 9001 localhost:9000");
    ...
    while (iter.hasNext()) {
      ...
      if (r.get("url") != null) {
        if (data.get(r.get("url")) != null) {
          ...
          if ((ref.get("responseCode") != null)
              && ((r.get("responseCode") == null) || !ref.get("responseCode").equals(r.get("responseCode")))) {
            problems = problems + "  * Response code for " + r.get("url") + " is " + r.get("responseCode")
                + ", should be " + ref.get("responseCode") + "\n";
          }
          if ((ref.get("contentType") != null)
              && ((r.get("contentType") == null) || !ref.get("contentType").equals(r.get("contentType")))) {
            problems = problems + "  * Content type for " + r.get("url") + " is " + r.get("contentType")
                + ", should be " + ref.get("contentType") + "\n";
          }
          if ((ref.get("length") != null)
              && ((r.get("length") == null) || !ref.get("length").equals(r.get("length")))) {
            problems = problems + "  * Length for " + r.get("url") + " is " + r.get("length") + ", should be "
                + ref.get("length") + "\n";
          }
  ...
  public static void main(String args[]) throws Exception {
    ...
    boolean runSetup = true, runTests = true, promptUser = true, outputToFile = false, exitUponFailure = true,
        cleanup = true;
    ...
    if ((args.length == 0) || args[0].equals("all") || args[0].equals("auto")
        || (args.length > 1) && (args[1].equals("all") || args[1].equals("auto"))) {
    }
