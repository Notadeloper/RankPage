...
public class Row implements Serializable {
  ...
  public static byte[] serializeRowBatch(List<Row> rowBatch) throws Exception {
      try (ByteArrayOutputStream baos = new ByteArrayOutputStream();
           ObjectOutputStream oos = new ObjectOutputStream(baos)) {
          
          oos.writeObject(rowBatch);
          byte[] rowBatchData = baos.toByteArray();
          return rowBatchData;
      }
  }
  
  public static List<Row> deserializeRowBatch(byte[] rowBatchData) throws Exception {
      try (ObjectInputStream ois = new ObjectInputStream(new ByteArrayInputStream(rowBatchData))) {
          @SuppressWarnings("unchecked")
          List<Row> rowBatch = (List<Row>) ois.readObject();
          return rowBatch;
      }
  }
}