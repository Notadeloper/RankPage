...
public class KVSClient implements KVS {
  ...
  public boolean lockRow(String tableName, String rowName) throws IOException {
      if (!haveWorkers) {
          downloadWorkers();
      }
      
      String target = "http://" + workers.elementAt(workerIndexForKey(rowName)).address + "/lock/" + tableName + "/"
              + URLEncoder.encode(rowName, "UTF-8");
      Response response = HTTP.doRequest("PUT", target, null);
      return response.statusCode() == 200;
  }
  
  public boolean unlockRow(String tableName, String rowName) throws IOException {
      if (!haveWorkers) {
          downloadWorkers();
      }
      
      String target = "http://" + workers.elementAt(workerIndexForKey(rowName)).address + "/unlock/" + tableName + "/"
              + URLEncoder.encode(rowName, "UTF-8");
      Response response = HTTP.doRequest("PUT", target, null);
      return response.statusCode() == 200;
  }
  ...
  public void putRowBatch(String tableName, List<Row> rows) throws Exception {
    if (!haveWorkers) {
      downloadWorkers();
    }

    Map<Integer, List<Row>> rowsGroupedByWorker = new HashMap<>();
    for (Row row : rows) {
      int index = workerIndexForKey(row.key());
      rowsGroupedByWorker.computeIfAbsent(index, i -> new ArrayList<>()).add(row);
    }

    for (Map.Entry<Integer, List<Row>> entry : rowsGroupedByWorker.entrySet()) {
      byte[] rowBatchData = Row.serializeRowBatch(entry.getValue());
      Response response = HTTP.doRequest("PUT",
          "http://" + workers.elementAt(entry.getKey()).address + "/batch/" + tableName, rowBatchData);
      byte[] responseByte = response.body();

      String result = new String(responseByte);
      logger.info("KVS worked returned" + result + " was the result" + response.statusCode() + " was status code");
      if (!result.equals("OK")) {
        throw new RuntimeException("Put returned something other than OK for an individual batch: " + result
            + "With code" + response.statusCode() + "was code");
      }
    }
  }

  public void putColInsertBatch(String tableName, List<ColInsert> colInserts) throws Exception {
    if (!haveWorkers) {
      downloadWorkers();
    }

    Map<Integer, List<ColInsert>> colInsertsGroupedByWorker = new HashMap<>();
    for (ColInsert colInsert : colInserts) {
      int index = workerIndexForKey(colInsert.rowID());
      colInsertsGroupedByWorker.computeIfAbsent(index, i -> new ArrayList<>()).add(colInsert);
    }

    for (Map.Entry<Integer, List<ColInsert>> entry : colInsertsGroupedByWorker.entrySet()) {
      byte[] colInsertBatchData = ColInsert.serializeColInsertBatch(entry.getValue());
      byte[] response = HTTP.doRequest("PUT",
          "http://" + workers.elementAt(entry.getKey()).address + "/colbatch/" + tableName, colInsertBatchData).body();

      String result = new String(response);
      if (!result.equals("OK")) {
        throw new RuntimeException("put returned something other than OK for an individual colInsert batch: " + result);
      }
    }
  }

  public synchronized Row getRow(String tableName, String row) throws IOException {
    if (!haveWorkers)
      downloadWorkers();
    
    //System.out.println(workers.size() + " " + workerIndexForKey(row) + " " + workers);
    /*
    if (workers.size() != 1) {
        System.out.println(workers.size() + " " + workerIndexForKey(row) + " " + workers);
    }*/
    ...
    int index = workerIndexForKey(row);
    HTTP.Response resp = HTTP.doRequest("GET", "http://" + workers.elementAt(index).address + "/data/"
  ...
  public synchronized boolean existsRow(String tableName, String row) throws FileNotFoundException, IOException {
    if (!haveWorkers)
      downloadWorkers();
    ...
    int index = workerIndexForKey(row);
    HTTP.Response r = HTTP.doRequest("GET", "http://" + workers.elementAt(index).address + "/data/"
        + tableName + "/" + java.net.URLEncoder.encode(row, "UTF-8"), null);
