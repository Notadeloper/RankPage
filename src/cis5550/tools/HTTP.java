    ...
    public static Response doRequestWithTimeout(String method, String urlArg, byte uploadOrNull[], int timeoutMillis,
        ...
        while (true) {
            ...
            try {
                        ...
                        for (int i = 0; i < inBuf; i++) {
                            ...
                            if (matchPtr == 2) {
                                ...
                                while (true) {
                                    ...
                                    String[] p2 = s.split(":", 2);
                                    if (p2.length == 2) {
                                        String headerName = p2[0].trim().toLowerCase();
                                        String headerValue = p2[1].trim();
                                        headers.put(headerName, headerValue);
                                        if (headerName.equals("content-length")) {
                                            try {
                                                contentLength = Integer.parseInt(headerValue);
                                            } catch (NumberFormatException e) {
                                                logger.info("Invalid content-length value in HTTPREQ: " + headerValue);
                                            }
                                        }
                                    }
                                }
                                ...
                                if (statusCode != 200) {
                                    logger.info(
                                            "Headers parsed in HTTPREQ. StatusCode: " + statusCode
                                                    + ", Content-Length: "
                                                    + contentLength + "");
                                    for (Map.Entry<String, String> entry : headers.entrySet()) {
                                        logger.info("key " + entry.getKey() + "value " + entry.getValue());
                                    }
                                    logger.info("was from this url:" + urlArg + "this method" + method);
                ...
                logger.info("exception occured in http req" + e);
                try {
                    sock.close();
                } catch (Exception e2) {
                }
                ...
                throw new IOException(
                        "Connection to " + host + ":" + port + " failed while reading the response (" + e + ")");
            }
